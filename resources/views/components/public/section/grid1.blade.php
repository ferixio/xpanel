...
@forelse ($data as $content)               
    <div>
      <div id="{{$content['id']}}" class="uk-card uk-card-default uk-border-rounded uk-overflow-hidden uk-transition-toggle uk-position-relative"  uk-responsive>
        <div  class="uk-card-media-top uk-background-cover content-img_thumb uk-height-medium  x-cursor"  data-src ="{{ is_null($content['image_thumb']) || empty($content['image_thumb']) ? asset('images/photo.svg')  : asset('storage/'.$content['image_thumb']) }}"  title="{{$content['title']}}" alt="{{$content['title']}}" uk-img>
          </div>
        <div class="uk-flex uk-flex-column uk-width-auto uk-position-top-right uk-position-small uk-padding-remove uk-overlay-default uk-overlay uk-box-shadow-small uk-transition-slide-right-small x-white-text uk-position-z-index">
          <a title="Whatsapp produk ini" href="#" class="uk-icon-button uk-text-success btn-whatsapp" uk-icon="whatsapp"  title="Chat via whatsapp" uk-tooltip="pos: left" ></a>
          <a title="Tentang produk ini"  href="{{url('product/detail/'.$content['slug'].'?id='.$content['id'])}}" class="uk-icon-button" uk-icon="info"  title="Detail Produk" uk-tooltip="pos: left" ></a>
          <a title="Masukan ke wishilist" href="#" class="uk-icon-button btn-add-wishlist" uk-icon="heart"  title="Tambah ke daftar suka" uk-tooltip="pos: left" ></a>
        </div>
        <div class="uk-card-body uk-text-left uk-text-baseline uk-padding-small uk-flex uk-child-width-expand">
          <div class="x-grid-desc">
            <a title="{{$content['title']}}" class="url_product" href="{{url('product/detail/'.$content['slug'].'?id='.$content['id'])}}" >
              <h2 class="uk-text-capitalize x-font-12 uk-width-expand uk-text-bold  uk-margin-remove content-title x-link ">{{ $content['title']}}</h2>
            </a>
          ...
          <div class="uk-position-bottom-right uk-width-auto" style="transform: translate(-10px,-10px)">
            <a title="cart" href="#" class="uk-icon-button btn-add-cart uk-text-bold" uk-icon="cart"  title="Tambah ke keranjang" uk-tooltip="pos: left" ></a>
          </div>
        </div>
      </div>
    </div>
