...
@forelse ($data as $content)               
    <div>
      <div id="{{$content['id']}}" class="uk-card uk-card-default uk-border-rounded uk-overflow-hidden uk-transition-toggle "  uk-responsive>
        ...
        <div class="uk-card-body uk-text-left uk-text-baseline uk-padding-small uk-flex uk-child-width-expand">
          <div class="x-grid-desc">
            <a class="url_product" href="{{url('product/detail/'.$content['slug'].'?id='.$content['id'])}}" >
              <h5 class="uk-text-capitalize x-font-16 uk-width-expand uk-text-bold  uk-margin-remove content-title x-link  ">{{ $content['title']}}</h5>
            </a>
            ...
            <p class="uk-text-capitalize x-font-12 uk-width-expand uk-text-bold uk-margin-remove content-price">
              @if ($content['price_promo'] > 0 )
              <span class="uk-width-expand  uk-margin-remove uk-text-danger" style="text-decoration: line-through;">Rp. {{ number_format($content['price'])}}</span> - 
          ...
          <div class="uk-width-auto">
            <a href="#" class="uk-icon-button btn-add-cart" uk-icon="cart"  title="Tambah ke keranjang" uk-tooltip="pos: left" ></a>
          </div>
        </div>
      </div>
    </div>
